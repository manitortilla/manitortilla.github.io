...
function updateGameInfo(){
  ...
  firebase.firestore().collection('userlist').doc(getUserUid()).get().then(function(doc){
    for (i=0; i<doc.data().game.length; i++)
      if (doc.data().game[i]['docid']==sessionStorage.gameID)
        manito = doc.data().game[i]['manitoof'];
      console.log(manito);
  });
  
  firebase.firestore().collection('gamelist').doc(sessionStorage.gameID).get().then(function(doc, manito){
...
function submitManito(){
  ...
  firebase.firestore().collection('userlist').doc(getUserUid()).get().then(function(doc){
    for (i=0; i<doc.data().game.length; i++)
      if (doc.data().game[i]['docid']==sessionStorage.gameID){
        temp=doc.data().game[i];
        break;
      }
    temp['manitoof']=document.getElementById("manitoid").value;
  });
  firebase.firestore().collection('userlist').doc(getUserUid()/sessionStorage.gameID).set({
      temp
  }).then(function(){ location.reload();});
}
